...
export const roomsHandler = (io: Server, socket: Socket) => {
	const getFamilyRoomsHandler = catchAsyncSocket(async (payload: { page: number }) => {
		const rooms = await getFamilyRooms(payload.page);

		socket.emit('recievedRooms', rooms);
	}, socket);

	const getRoomsByNameHandler = catchAsyncSocket(async (payload: { roomName: string }) => {
	...
	const createRoomHandler = catchAsyncSocket(async (payload: CreateRoomSchemaType) => {
	...
	const deleteRoomHandler = catchAsyncSocket(async (payload: DeleteRoomSchemaType) => {
	...
	const joinRoomHandler = catchAsyncSocket(async (payload: JoinRoomSchemaType) => {
