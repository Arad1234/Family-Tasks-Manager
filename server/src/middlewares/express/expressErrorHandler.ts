...
import { handleCastErrorDB, handleDuplicateFieldsDB, handleValidationErrorDB } from '../../utils/errorHandlerFunctions';
...
const sendErrorProd = (err: any, res: Response) => {
	...
	// Programming or other unkown error: don't leak error details.
		...
		res.status(INTERNAL_SERVER_ERROR).json({ status: 'error', message: 'Something went very wrong!' });
	}
};

// Express error middleware.
export const expressErrorHandler = (err: any, _req: Request, res: Response, _next: NextFunction) => {
	err.statusCode = err.statusCode || INTERNAL_SERVER_ERROR;
	err.status = err.status || 'error';

	if (config.environment.nodeEnv === 'development') {
		sendErrorDev(err, res);
	} else if (config.environment.nodeEnv === 'production') {
