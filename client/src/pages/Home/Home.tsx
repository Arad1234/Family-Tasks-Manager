...
import SearchInput from '@Components/Home-UI/Search/SearchInput';

export const Home = () => {
	const dispatch = useAppDispatch();

	const page = useAppSelector((state) => state.paginationReducer.page);
	const isSearchRoom = useAppSelector((state) => state.roomsReducer.isSearchRoom);
	...
	return loading || !socket.connected ? (
		...
		<>
			...
			<Box sx={{ display: 'flex', justifyContent: 'center' }}>
				{isSearchRoom ? <Loader height='88vh' /> : <AllRooms ref={callBackRef} />}
			</Box>
		</>
	);
};
